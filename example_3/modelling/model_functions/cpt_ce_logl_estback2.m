function logl = cpt_ce_logl_est(x,p,y,q, choice, params)
% x = [-10, -20, 30; 12 15 40];
...
% params 2 = gain pwt
% params 3 = loss pwt
% params 4 = loss aversion
% params 5 = sensitivity
if length(params) == 3
    ...
    if sum(x(i,:)<0)==0
        ...
        if length(xgains) == 1
            ...
            for k = 2:length(xgains)
                xutils(k) = (xgains(k).^params(1)).*pwt(sum(pgains(1:k)) - sum(pgains(1:k-1)), params(3));
            end
        end
        xutil(i) = sum(xutils);
    elseif sum(x(i,:)>0) == 0
        ...
        if length(xlosses) == 1
            ...
            for k = 1:length(xlosses)
                xutils(k) = -(abs(xlosses(k)).^params(2)).*pwt(sum(plosses(1:k)) - sum(plosses(1:k-1)), params(4));
            end
        end
    ...
    if sum(y(i,:)<0)==0
        ...
        if length(ygains) == 1
            ...
            for k = 2:length(ygains)
                yutils(k) = (ygains(k).^params(1)).*pwt(sum(qgains(1:k)) - sum(qgains(1:k-1)), params(3));
            end
        end
        yutil(i) = sum(yutils);
    elseif sum(y(i,:)>0) == 0
        ...
        if length(ylosses) == 1
            ...
            for k = 1:length(ylosses)
                yutils(k) = -(abs(ylosses(k)).^params(2)).*pwt(sum(qlosses(1:k)) - sum(qlosses(1:k-1)), params(4));
            end
        end
...
endxutil(xutil>=0) = xutil(xutil>=0).^(1./params(1));
