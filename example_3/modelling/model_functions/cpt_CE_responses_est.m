function [logl, xutil] = cpt_CE_responses_est(x,p, choice, params)
% x = [-10, -20, 30; 12 15 40];
...
%     disp('Loss aversion parameter has been manually turned off in function')
    params = [params(1), params(1), params(2), params(2), 1, params(3)];
...
% logl = sum(((choice - xutil).*params(6)).^2);
% logl = sum(1-(normpdf(choice, xutil, params(6))./normpdf(0,0,params(6))));
% logl = sum(1-(normpdf(choice, xutil, params(6))));

% logl = -sum(log(normpdf(choice, xutil, params(6))));
logl = -sum(log(normpdf(choice, xutil, params(6))));
% logl = -sum(log(normpdf((choice - xutil)./params(6), 0, params(6))));
