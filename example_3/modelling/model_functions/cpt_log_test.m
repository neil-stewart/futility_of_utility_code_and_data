...
params = [0.88 0.88 0.61 0.69 2.25 1];

params = [0.88 0.61 2.25 0.001];
...
xutil_ce = xutil;
yutil_ce = yutil;
xutil_ce(xutil>=0) = xutil(xutil>=0).^(1./params(1));
xutil_ce(xutil<0) = -abs(xutil(xutil<0)./params(5)).^(1./params(2));
yutil_ce(yutil>=0) = yutil(yutil>=0).^(1./params(1));
yutil_ce(yutil<0) = -abs(yutil(yutil<0)./params(5)).^(1./params(2));

preds = 1./(1 + exp(-(xutil - yutil).*params(6)));
preds = 1./(1 + exp(-(xutil_ce - yutil_ce).*params(6)));
% logl1 = -(sum(log(preds).*choice') + sum(log(1-preds).*(1-choice')))
