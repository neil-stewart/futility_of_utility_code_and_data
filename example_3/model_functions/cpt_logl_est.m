function logl = cpt_logl_est(x,p,y,q, choice, params)
% x = [-10, -20, 30; 12 15 40];
...
% xutil(xutil<0) = -xutil(xutil<0).^(1./params(2));
% yutil(yutil>=0) = yutil(yutil>=0).^(1./params(1));
% yutil(yutil<0) = -yutil(yutil<0).^(1./params(2));

preds = 1./(1 + exp(-(xutil - yutil).*params(6)));
logl = -(sum(log(preds(choice == 1))) + sum(log(1-preds(choice == 0))));% xutil
end
